...
/-! # `add'_eq_add`, part 1/3 — supporting lemmas (no heavy certificates).
...
noncomputable def addCoords (E : Edwards (𝔽 p)) (x₁ y₁ x₂ y₂ : 𝔽 p) : 𝔽 p × 𝔽 p :=
...
theorem lam_sq_ne_one (E : Edwards (𝔽 p)) {x₁ y₁ x₂ y₂ : 𝔽 p}
  ...
  · by_cases hcaseNeg : a' * x₂ - y₂ = 0
    · have h2x : 2 * a' * x₂ = 0 := by linear_combination hcasePos + hcaseNeg
      exact hx2 ((mul_eq_zero.mp h2x).resolve_left (mul_ne_zero Montgomery.two_ne_zero ha'0))
...
/-- The Edwards addition-law denominators `1 ± d·x₁x₂y₁y₂` are nonzero (completeness). -/
theorem denoms_ne_zero (E : Edwards (𝔽 p)) {x₁ y₁ x₂ y₂ : 𝔽 p}
    (h1 : E.Equation x₁ y₁) (h2 : E.Equation x₂ y₂) :
...
theorem addCoords_onCurve (E : Edwards (𝔽 p)) {x₁ y₁ x₂ y₂ : 𝔽 p}
    (h1 : E.Equation x₁ y₁) (h2 : E.Equation x₂ y₂) :
  ...
  have hpos' : (1 : 𝔽 p) + x₁ * y₂ * y₁ * x₂ * E.d ≠ 0 := by rw [e]; exact hpos
  have hneg' : (1 : 𝔽 p) - x₁ * y₂ * y₁ * x₂ * E.d ≠ 0 := by rw [e]; exact hneg
  rw [Edwards.Equation] at h1 h2
  simp only [Edwards.addCoords, Edwards.Equation]
...
/-- The explicit Edwards addition as an operation on points (an implementation's `add`). -/
noncomputable def add' (E : Edwards (𝔽 p)) : E.Point → E.Point → E.Point
  | .mk x₁ y₁ h1, .mk x₂ y₂ h2 =>
...
identity case of `pointEquiv_add'`). -/
theorem addCoords_zero_left (E : Edwards (𝔽 p)) (x y : 𝔽 p) : E.addCoords 0 1 x y = (x, y) := by
  simp [Edwards.addCoords]

omit [Fact (2 < p)] in
/-- Adding the identity `(0,1)` on the right returns the point. -/
theorem addCoords_zero_right (E : Edwards (𝔽 p)) (x y : 𝔽 p) : E.addCoords x y 0 1 = (x, y) := by
  simp [Edwards.addCoords]

/-- `(−x, y)` is the explicit inverse: `add'` with it returns the identity `(0,1)`. -/
theorem addCoords_inv (E : Edwards (𝔽 p)) {x y : 𝔽 p}
    (h : E.Equation x y) (h' : E.Equation (-x) y) : E.addCoords x y (-x) y = (0, 1) := by
  obtain ⟨_, hden⟩ := E.denoms_ne_zero h h'
  rw [Edwards.Equation] at h
  simp only [Edwards.addCoords, Prod.mk.injEq]
  refine ⟨?_, ?_⟩
  · rw [div_eq_zero_iff]; left; ring
  · rw [div_eq_one_iff_eq hden]; linear_combination h

/-- **Computation lemma**: the bridge sends the Edwards identity `(0,1)` to the Weierstrass zero. -/
theorem pointEquiv_zero_one (E : Edwards (𝔽 p)) (h : E.Equation 0 1) :
    E.pointEquiv (Edwards.Point.mk 0 1 h) = 0 := by
  simp only [Edwards.pointEquiv, Equiv.trans_apply, Edwards.pointEquivMont,
    Montgomery.pointEquiv, Equiv.coe_fn_mk, dif_pos, if_pos]
  rfl

omit [Fact (2 < p)] in
/-- `toMontV` is odd in `x`: `toMontV (−x) y = −toMontV x y`. -/
theorem toMontV_neg {x y : 𝔽 p} (hx : x ≠ 0) (hy : (1 : 𝔽 p) - y ≠ 0) :
...
Edwards point `(x, y)` to the Weierstrass point with coordinates `(B · toMontU y, B² · toMontV x y)`
(`B = E.toMontgomery.B`), via Edwards → Montgomery → Weierstrass. -/
theorem pointEquiv_some (E : Edwards (𝔽 p)) {x y : 𝔽 p} (h : E.Equation x y) (hx : x ≠ 0) :
    E.pointEquiv (Edwards.Point.mk x y h) =
      .some (E.toMontgomery.B * toMontU y) (E.toMontgomery.B ^ 2 * toMontV x y)
        ((WeierstrassCurve.Affine.equation_iff_nonsingular).mp
          (Montgomery.equation_toW E.toMontgomery (E.forward_onCurve h hx))) := by
  simp only [Edwards.pointEquiv, Equiv.trans_apply, Edwards.pointEquivMont, Montgomery.pointEquiv,
    Equiv.coe_fn_mk, dif_neg hx]

/-- **The bridge respects negation**: `pointEquiv (−x, y) = −(pointEquiv (x, y))`
(Edwards negation is `(x,y) ↦ (−x, y)`; on the W side `negY = −y` as `a₁ = a₃ = 0`). -/
theorem pointEquiv_neg (E : Edwards (𝔽 p)) {x y : 𝔽 p}
    (h : E.Equation x y) (h' : E.Equation (-x) y) :
    E.pointEquiv (Edwards.Point.mk (-x) y h') = -(E.pointEquiv (Edwards.Point.mk x y h)) := by
  by_cases hx : x = 0
  · subst hx
    ...
    · -- the `(0,−1)`-type 2-torsion point: `pointEquiv = some 0 0 = −(some 0 0)`
      have key : E.pointEquiv (Edwards.Point.mk 0 y h)
          = -(E.pointEquiv (Edwards.Point.mk 0 y h)) := by
        have hc : E.pointEquiv (Edwards.Point.mk 0 y h)
            = (E.toMontgomery).pointEquiv (.some 0 0 (by simp [Montgomery.Equation])) := by
          simp only [Edwards.pointEquiv, Equiv.trans_apply, Edwards.pointEquivMont,
            Equiv.coe_fn_mk, dif_pos, if_neg hy1]
        rw [hc]
        simp only [Montgomery.pointEquiv, Equiv.coe_fn_mk, mul_zero,
          WeierstrassCurve.Affine.Point.neg_some, WeierstrassCurve.Affine.Point.some.injEq,
          true_and]
        simp only [WeierstrassCurve.Affine.negY, Montgomery.toWeierstrass]; ring
      exact key
  · rw [pointEquiv_some E h' (neg_ne_zero.mpr hx), pointEquiv_some E h hx,
      WeierstrassCurve.Affine.Point.neg_some, WeierstrassCurve.Affine.Point.some.injEq]
    refine ⟨rfl, ?_⟩
    rw [toMontV_neg hx (E.one_sub_y_ne_zero h hx)]
    simp only [WeierstrassCurve.Affine.negY, Montgomery.toWeierstrass]
    ring
...
theorem pointEquiv_eq_some (E : Edwards (𝔽 p)) {x y : 𝔽 p}
    (h : E.Equation x y) (hne : ¬(x = 0 ∧ y = 1)) :
    ∃ hns, E.pointEquiv (Edwards.Point.mk x y h)
      = .some (E.toMontgomery.B * toMontU y) (E.toMontgomery.B ^ 2 * toMontV x y) hns := by
  by_cases hx : x = 0
  · subst hx
    have hy : y = -1 := by
      have hy2 : y ^ 2 = 1 := by rw [Edwards.Equation] at h; linear_combination h
    ...
    subst hy
    have hne1 : (-1 : 𝔽 p) ≠ 1 := fun hh => Montgomery.two_ne_zero (by linear_combination -hh)
    have hu : toMontU (-1 : 𝔽 p) = 0 := by unfold toMontU; norm_num
    have hv : toMontV (0 : 𝔽 p) (-1) = 0 := by unfold toMontV; simp
    ...
    · have hc : E.pointEquiv (Edwards.Point.mk 0 (-1) h)
          = (E.toMontgomery).pointEquiv (.some 0 0 (by simp [Montgomery.Equation])) := by
        simp only [Edwards.pointEquiv, Equiv.trans_apply, Edwards.pointEquivMont, Equiv.coe_fn_mk,
          dif_pos, if_neg hne1]
      rw [hc]
      simp only [Montgomery.pointEquiv, Equiv.coe_fn_mk, mul_zero]
  · exact ⟨_, pointEquiv_some E h hx⟩

/-! ### The four cases of `pointEquiv_add'` (each a separate lemma) -/

/-- **Case 1a (identity, left)** — EASY: `add' (0,1) Q = Q` and `pointEquiv (0,1) = 0`. -/
theorem pointEquiv_add'_id_left (E : Edwards (𝔽 p)) {x₂ y₂ : 𝔽 p}
    (h01 : E.Equation 0 1) (h2 : E.Equation x₂ y₂) :
    E.pointEquiv (E.add' (.mk 0 1 h01) (.mk x₂ y₂ h2))
      = E.pointEquiv (.mk 0 1 h01) + E.pointEquiv (.mk x₂ y₂ h2) := by
  have hadd : E.add' (.mk 0 1 h01) (.mk x₂ y₂ h2) = .mk x₂ y₂ h2 := by
    simp only [Edwards.add', E.addCoords_zero_left]
  rw [hadd, pointEquiv_zero_one, zero_add]

/-- **Case 1b (identity, right)** — EASY: `add' P (0,1) = P`. -/
theorem pointEquiv_add'_id_right (E : Edwards (𝔽 p)) {x₁ y₁ : 𝔽 p}
    (h1 : E.Equation x₁ y₁) (h01 : E.Equation 0 1) :
    E.pointEquiv (E.add' (.mk x₁ y₁ h1) (.mk 0 1 h01))
      = E.pointEquiv (.mk x₁ y₁ h1) + E.pointEquiv (.mk 0 1 h01) := by
  have hadd : E.add' (.mk x₁ y₁ h1) (.mk 0 1 h01) = .mk x₁ y₁ h1 := by
...
theorem pointEquiv_add'_inverse (E : Edwards (𝔽 p)) {x y : 𝔽 p}
  ...
  have hadd : E.add' (.mk x y h) (.mk (-x) y h') = .mk 0 1 (by simp [Edwards.Equation]) := by
    simp only [Edwards.add', E.addCoords_inv h h']
...
theorem generic_notWInv (E : Edwards (𝔽 p)) {x₁ y₁ x₂ y₂ : 𝔽 p}
    (h1 : E.Equation x₁ y₁) (h2 : E.Equation x₂ y₂)
  ...
  have h2' : E.Equation (-x₂) y₂ := by rw [Edwards.Equation] at h2 ⊢; linear_combination h2
