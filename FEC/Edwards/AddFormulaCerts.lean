...
/-! # `add'_eq_add`, part 2/3 — the coordinate-match certificates (pieces C and D).
...
theorem generic_addX (E : Edwards (𝔽 p)) {x₁ y₁ x₂ y₂ : 𝔽 p}
  ...
  have h1y : (1 : 𝔽 p) - y₁ ≠ 0 := sub_ne_zero.mpr (Ne.symm (E.y_ne_one h1 hP))
  have h2y : (1 : 𝔽 p) - y₂ ≠ 0 := sub_ne_zero.mpr (Ne.symm (E.y_ne_one h2 hQ))
  ...
  · -- **2-torsion input** `P = (0, -1)`.
    ...
    have hx2 : x₂ ≠ 0 := by
      ...
      rcases mul_eq_zero.mp (show (y₂ - 1) * (y₂ + 1) = 0 by
        rw [Edwards.Equation] at h2; linear_combination h2) with h | h
      · exact hQ ⟨rfl, by linear_combination h⟩
      · exact hinv ⟨by ring, by linear_combination h⟩
    have h4ne : (4 : 𝔽 p) ≠ 0 := by
      rw [show (4 : 𝔽 p) = 2 * 2 by norm_num]
      exact mul_ne_zero Montgomery.two_ne_zero Montgomery.two_ne_zero
    have hBne : E.toMontgomery.B ≠ 0 := by
      have : E.toMontgomery.B = 4 / (E.a - E.d) := rfl
      rw [this]; exact div_ne_zero h4ne had
    have hU1 : toMontU (-1 : 𝔽 p) = 0 := by unfold toMontU; norm_num
    have hV1 : toMontV (0 : 𝔽 p) (-1) = 0 := by unfold toMontV; norm_num
    have hy2nm1 : y₂ ≠ -1 := by
      intro h; apply hx2
      rw [Edwards.Equation, h] at h2
  ...
  · -- **2-torsion input** `Q = (0, -1)`.
    ...
    have hBne : E.toMontgomery.B ≠ 0 := by
      have : E.toMontgomery.B = 4 / (E.a - E.d) := rfl
      rw [this]; exact div_ne_zero h4ne had
    have hU2 : toMontU (-1 : 𝔽 p) = 0 := by unfold toMontU; norm_num
    have hV2 : toMontV (0 : 𝔽 p) (-1) = 0 := by unfold toMontV; norm_num
    have hy1nm1 : y₁ ≠ -1 := by
      intro h; apply hx1
      rw [Edwards.Equation, h] at h1
  ...
  · -- **Doubling** `P = Q`: `U₁=U₂ ⟹ y₂=y₁` (injectivity), then `¬inv` + curve ⟹ `x₂=x₁`.
    ...
    have hy : y₂ = y₁ := by
      ...
      have h20 : (2 : 𝔽 p) * (y₂ - y₁) = 0 := by linear_combination -hUeq
      linear_combination (mul_eq_zero.mp h20).resolve_left Montgomery.two_ne_zero
    subst y₂
    have hy1nm1 : y₁ ≠ -1 := by
    ...
    have hy1p : (1 : 𝔽 p) + y₁ ≠ 0 := fun h => hy1nm1 (by linear_combination h)
    have hnegD : (1 : 𝔽 p) - E.d * y₁ ^ 2 * x₁ ^ 2 ≠ 0 := by
      rw [show (1 : 𝔽 p) - E.d * y₁ ^ 2 * x₁ ^ 2 = 1 - E.d * x₁ * x₁ * y₁ * y₁ from by ring]
      exact hneg
    have hsumnumD : (1 : 𝔽 p) - E.d * y₁ ^ 2 * x₁ ^ 2 - (y₁ ^ 2 - E.a * x₁ ^ 2) ≠ 0 := by
      intro hh
      refine (E.y_ne_one (E.addCoords_onCurve h1 h2) (E.generic_sumNeId h1 h2 hinv)) ?_
      simp only [Edwards.addCoords]
      rw [div_eq_one_iff_eq (by rw [show (1 : 𝔽 p) - E.d * x₁ * x₁ * y₁ * y₁
  ...
  · rw [WeierstrassCurve.Affine.slope_of_X_ne hUU]
    ...
    have hneg' : (1 : 𝔽 p) - E.d * y₁ * y₂ * x₁ * x₂ ≠ 0 := by
      rw [show (1 : 𝔽 p) - E.d * y₁ * y₂ * x₁ * x₂ = 1 - E.d * x₁ * x₂ * y₁ * y₂ from by ring]
      exact hneg
    have hslopeden : (1 + y₁) * (1 - y₂) - (1 - y₁) * (1 + y₂) ≠ 0 := by
      rw [show (1 + y₁) * (1 - y₂) - (1 - y₁) * (1 + y₂) = 2 * (y₁ - y₂) from by ring]
      exact mul_ne_zero Montgomery.two_ne_zero (sub_ne_zero.mpr hyne)
    have hsumnum : (1 : 𝔽 p) - E.d * y₁ * y₂ * x₁ * x₂ - (y₁ * y₂ - E.a * x₁ * x₂) ≠ 0 := by
      intro hh
      refine (E.y_ne_one (E.addCoords_onCurve h1 h2) (E.generic_sumNeId h1 h2 hinv)) ?_
      simp only [Edwards.addCoords]
      rw [div_eq_one_iff_eq (by rw [show (1 : 𝔽 p) - E.d * x₁ * x₂ * y₁ * y₂
...
theorem generic_addY_secant (E : Edwards (𝔽 p)) {x₁ y₁ x₂ y₂ : 𝔽 p}
    (h1 : E.Equation x₁ y₁) (h2 : E.Equation x₂ y₂)
    (hP : ¬(x₁ = 0 ∧ y₁ = 1)) (hQ : ¬(x₂ = 0 ∧ y₂ = 1)) (hinv : ¬(x₂ = -x₁ ∧ y₂ = y₁))
  ...
  have hBne : E.toMontgomery.B ≠ 0 := by
    have : E.toMontgomery.B = 4 / (E.a - E.d) := rfl
    rw [this]; exact div_ne_zero h4ne had
  have hU1m : toMontU (-1 : 𝔽 p) = 0 := by unfold toMontU; norm_num
  have hV1m : toMontV (0 : 𝔽 p) (-1) = 0 := by unfold toMontV; norm_num
  rw [WeierstrassCurve.Affine.slope_of_X_ne hUU]
  have hyne : y₁ ≠ y₂ := fun h => hUU (by rw [h])
  have hneg' : (1 : 𝔽 p) - E.d * y₁ * y₂ * x₁ * x₂ ≠ 0 := by
    rw [show (1 : 𝔽 p) - E.d * y₁ * y₂ * x₁ * x₂ = 1 - E.d * x₁ * x₂ * y₁ * y₂ from by ring]
  ...
  · -- `sum = (0, -1)` via secant. Completeness forces `y₂ = -y₁`, then `sumX = 0 ⟹ x₂ = x₁`;
    ...
    have hSX0 : (E.addCoords x₁ y₁ x₂ y₂).1 = 0 := by
      simp only [Edwards.addCoords]; rw [div_eq_zero_iff]; exact Or.inl hsX
    have hSY : (E.addCoords x₁ y₁ x₂ y₂).2 = -1 := by
      have hon := E.addCoords_onCurve h1 h2
      rw [Edwards.Equation, hSX0] at hon
    ...
    have gsY : y₁ * y₂ - E.a * x₁ * x₂ + 1 - E.d * x₁ * x₂ * y₁ * y₂ = 0 := by
      have ht := hSY; simp only [Edwards.addCoords] at ht
    ...
    have hAX0 : (E.toMontgomery.toWeierstrass).toAffine.addX (E.toMontgomery.B * toMontU y₁)
      ...
      exact hC.symm
    rw [hSX0, show toMontV (0 : 𝔽 p) (E.addCoords x₁ y₁ x₁ (-y₁)).2 = 0 from by simp [toMontV],
      mul_zero]
...
theorem generic_addY (E : Edwards (𝔽 p)) {x₁ y₁ x₂ y₂ : 𝔽 p}
  ...
  · -- **Doubling** `P = Q`.
    ...
    · -- `2P = (0, -1)`: `sumX = 0`, so `toMontV(0,·)=0` and LHS `= 0`; show `addY = 0`.
      subst hy10
      have hsX0 : (E.addCoords x₁ (0 : 𝔽 p) x₁ 0).1 = 0 := by simp [Edwards.addCoords]
      rw [hsX0]
      simp only [Edwards.toMontgomery, Montgomery.toWeierstrass, WeierstrassCurve.Affine.addY,
        WeierstrassCurve.Affine.negAddY, WeierstrassCurve.Affine.addX, WeierstrassCurve.Affine.negY,
        toMontU, toMontV, Edwards.addCoords, mul_zero, zero_mul, sub_zero, add_zero, div_zero,
        zero_div]
      field_simp
      rw [Edwards.Equation] at h1
